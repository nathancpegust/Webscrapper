...
def main():
    ...
    data_transformer.create_new_table_for_article_and_content(
        'standard_news_article_table', 'standard_news_content_table')
    src = "The Standard"
    ...
    while (True):
        ...
        for element in data["htmlData"]:
            ...
            print(soup)
            news_paragraphs = soup.find(
                "div", {"class": "content"}).find_all("p")
            for news_paragraph in news_paragraphs:
                if "</a>" not in (str(news_paragraph)):
                    content.append(str(news_paragraph).strip(
                        "<p>").strip("</p>").strip("\r"))
            newsResult = {
                'keyword': str(keyword),
